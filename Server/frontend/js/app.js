...
function handleWelcome(msg) {
    ...
    document.getElementById("welcome-msg").textContent = `Welcome, ${msg.username}!`;
    
    // Only show host controls if quiz hasn't started yet
    if (msg.username === msg.host && !msg.quiz_started) {
...
function startSyncCountdown(startTs) {
    ...
    document.querySelector("#waiting-screen .loader").style.display = "none";
...
function handleQuestionClosed(payload) {
    ...
    if (gotItRight) {
        ...
        updateStatus(`Time's up! The correct answer was: ${payload.answer}`, "#ef4444");
    }
    
    // Show leaderboard after a brief delay
    setTimeout(() => {
        // Only switch if we are still theoretically in the quiz screen (handles race conditions)
        if (Object.values(screens).find(s => s.classList.contains("active")) === screens.quiz) {
            showScreen("finished");
            document.getElementById("finished-title").textContent = "Round Leaderboard";
...
document.getElementById("join-btn").addEventListener("click", () => {
    ...
    if (ws && ws.readyState === WebSocket.OPEN) {
        ...
        document.getElementById("join-error").textContent = "Socket not connected yet.";
    }
});
